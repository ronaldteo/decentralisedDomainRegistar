...
const SendEth = () => {
  ...
  const features = [
    { title: 'Direct Transfer', description: 'Send ETH directly to .ntu domain names' },
    { title: 'No Address Needed', description: 'Just enter the domain, we handle the rest' },
    { title: 'Instant Settlement', description: 'Transactions confirmed on the blockchain' },
  ];
        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {features.map((feature, idx) => (
              <Card key={idx} className="border-gray-300 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg">
                <CardHeader className="pb-4">
                  <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full border-2 border-gray-300 bg-gray-50">
                    <span className="text-2xl font-bold leading-none text-black">{idx + 1}</span>
                  </div>
                  <CardTitle className="mb-2 text-xl leading-tight text-black">{feature.title}</CardTitle>
