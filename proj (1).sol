...
contract NTUDomainRegistrar {
    ...
    function finalizeAuction(string memory domainName) public {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            reverseRegistry[auction.highestBidder].push(domainName);
        ...
        domainRegistered(domainName)
    {
        ...
        _removeDomainFromOwner(oldOwner, domainName);
        reverseRegistry[newOwner].push(domainName);
        
        emit DomainTransferred(domainName, oldOwner, newOwner);
    }
    
    function _removeDomainFromOwner(address owner, string memory domainName) 
        internal 
    {
        string[] storage domainList = reverseRegistry[owner];
        for (uint i = 0; i < domainList.length; i++) {
            if (keccak256(bytes(domainList[i])) == keccak256(bytes(domainName))) {
